...
import (
	...
	"github.com/qi0523/distribution-agent/manifest"
)
...
func init() {
	schema1Func := func(b []byte) (manifest.Manifest, manifest.Descriptor, error) {
	...
	err := manifest.RegisterManifestSchema(MediaTypeSignedManifest, schema1Func)
	if err != nil {
		panic(fmt.Sprintf("Unable to register manifest: %s", err))
	}
	err = manifest.RegisterManifestSchema("", schema1Func)
	if err != nil {
		panic(fmt.Sprintf("Unable to register manifest: %s", err))
	}
	err = manifest.RegisterManifestSchema("application/json", schema1Func)
	if err != nil {
		panic(fmt.Sprintf("Unable to register manifest: %s", err))
	}
}
...
func (sm SignedManifest) References() []manifest.Descriptor {
	dependencies := make([]manifest.Descriptor, len(sm.FSLayers))
	for i, fsLayer := range sm.FSLayers {
		dependencies[i] = manifest.Descriptor{
